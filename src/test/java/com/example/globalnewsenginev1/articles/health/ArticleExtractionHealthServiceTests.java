...
class ArticleExtractionHealthServiceTests {
    ...
    void setUp() throws Exception {
        ...
        jdbcTemplate.execute("""
                CREATE TABLE gdelt_gkg (
                    ...
                    source_timestamp TIMESTAMP WITH TIME ZONE NOT NULL, created_at TIMESTAMP WITH TIME ZONE NOT NULL)
                """);
    ...
    private long insertStageMention(Instant timestamp, long rowNumber) {
        long payloadId = insertRaw("MENTIONS", "gdelt_mention_payloads", timestamp, rowNumber);
        jdbcTemplate.update("""
    ...
    private void insertStageGkg(Instant timestamp, long rowNumber) {
        long rawId = insertRaw("GKG", "gdelt_gkg_payloads", timestamp, rowNumber);
        jdbcTemplate.update("""
                INSERT INTO gdelt_gkg (id, source_timestamp, created_at)
                SELECT id, source_timestamp, ? FROM gdelt_gkg_payloads WHERE id = ?
                """, utc(timestamp), rawId);
    }
