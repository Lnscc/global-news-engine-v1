...
class GdeltMentionPayloadMigrationTests {
    ...
    void preservesMentionIdentityAndRemapsArticleReferences() throws Exception {
        ...
        jdbcTemplate.update("""
                ...
                VALUES (5, 7, 'mentions.zip', ?, 2, 'second parsed raw row', ?),
                       (42, 7, 'mentions.zip', ?, 3, 'unchanged raw row', ?),
                       (43, 7, 'mentions.zip', ?, 4, 'unparsed raw row', ?)
                """, ingestedAt, ingestedAt, ingestedAt, ingestedAt, ingestedAt, ingestedAt);
        jdbcTemplate.update("""
                ...
                VALUES (5, 42, 7, 'mentions.zip', ?, 3, ?, 123, 1,
                       ...
                       (42, 5, 7, 'mentions.zip', ?, 2, ?, 124, 1,
                        'https://example.org/second', 90, -0.5)
                """, ingestedAt, parsedAt, ingestedAt, parsedAt);
        jdbcTemplate.update("""
        ...
        jdbcTemplate.update("""
                ...
                VALUES ('MENTIONS', 5, ?, 'INVALID_URL', 'legacy failure', ?),
                       ('MENTIONS', 42, ?, 'INVALID_URL', 'second legacy failure', ?)
                """, ingestedAt, ingestedAt, ingestedAt, ingestedAt);
        ...
        assertThat(jdbcTemplate.queryForMap("SELECT * FROM gdelt_mentions WHERE id = 42"))
                .containsEntry("ID", 42L)
        ...
        assertThat(jdbcTemplate.queryForList(
                "SELECT source_id FROM article_signals WHERE signal_type = 'MENTIONS' ORDER BY source_id",
                Long.class)).containsExactly(5L, 42L);
        assertThat(jdbcTemplate.queryForList(
                "SELECT source_id FROM article_extraction_errors WHERE signal_type = 'MENTIONS' ORDER BY source_id",
                Long.class)).containsExactly(5L, 42L);
